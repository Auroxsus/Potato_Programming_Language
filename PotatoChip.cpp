...
void ParseSWITCHStatement(TOKEN tokens[]) {
    ...
    GetNextToken(tokens); // eat SWITCH
    if (tokens[0].type != OPARENTHESE) {
        ProcessCompilerError(tokens[0].sourceLineNumber, tokens[0].sourceLineIndex, "Expecting '('");
    }
    GetNextToken(tokens); // eat '('

    if (tokens[0].type == ONE || tokens[0].type == ALL) {
        mode = (tokens[0].type == ALL) ? 'A' : 'O';
        GetNextToken(tokens); // eat mode
    } else {
        mode = 'O';  // Default mode is ONE
    }
    ...
    // Parse CASE and DEFAULT blocks
    while (tokens[0].type == CASE || tokens[0].type == DEFAULT) {
        if (tokens[0].type == CASE) {
            GetNextToken(tokens); // eat CASE
            ParseExpression(tokens, datatype);
            ...
            // Parse statements in the CASE block
            while (tokens[0].type != CASE && tokens[0].type != DEFAULT && tokens[0].type != CBRACE) {
                if (tokens[0].type == BREAK) {  // Handle 'BREAK'
                    GetNextToken(tokens); // eat SERVE
                    if (tokens[0].type != SEMICOLON) {
                        ProcessCompilerError(tokens[0].sourceLineNumber, tokens[0].sourceLineIndex, "Expecting ';' after SERVE");
                    }
                    GetNextToken(tokens); // eat ';'
            ...
            GetNextToken(tokens); // eat DEFAULT
